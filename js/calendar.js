...
function init(SY, EY) {
	...
	CALS[0].innerHTML = y + "年";
	CALS[0].setAttribute("value", y);

	CALS[1].innerHTML = y + "年假日安排";
	CALS[1].setAttribute("value", 0);
	var L0 = document.createElement("li");
	L0.setAttribute("value", 0);
	L0.innerHTML = y + "年假日安排";
	ULS[1].appendChild(L0);

	CALS[2].innerHTML = m + "月";
	CALS[2].setAttribute("value", m);
	...
	for (var i = 0; i < calendarData.Y_Holidays.length; i++) {
		...
		LI_H.innerHTML = calendarData.Y_Holidays[i].n;
		LI_H.setAttribute("date", calendarData.Y_Holidays[i].d);
		LI_H.setAttribute("value", i + 1);
	...
	for (var i = 0; i < ULS.length; i++) {
		(function() {
			...
			//模拟hover效果
			for (var m = 0; m < ULS[j].childNodes.length; m++) {
				ULS[j].childNodes[m].onmouseenter = function(e) {
					e = window.event || e;
					var target = e.target || e.srcElement;
					addClass(target, "hover");
				}

				ULS[j].childNodes[m].onmouseleave = function(e) {
					e = window.event || e;
					var target = e.target || e.srcElement;
...
function toggle(event, index) {
	if (index >= 3) {
		...
		var times = parseInt(CALS[index].getAttribute("value")) - parseInt(ULS[index].childNodes[0].getAttribute("value"));
		ULS[index].scrollTop = (ULS[index].childNodes[0].offsetHeight + 0.5) * times;
		addClass(ULS[index].childNodes[times], "hover");
	}
...
function generateCalendar(year, month) {
	...
	for (var i = 0; i < array_date.length; i++) {
		...
		} else if (LDate.getMonth() == 11) {
			var LMS = getLunarMonths(LDate.getFullYear());
			if (LMS[LMS.length - 1].c == LDate.getDate()) {
				addClass(DLi, "lunarfestival");
				div_lunar.innerHTML = "除夕";
			} else
		...
		for (var j = 0; j < calendarData.Y_Holidays.length; j++) {
			var hDate = new Date(calendarData.Y_Holidays[j].d);
			var hY = hDate.getFullYear();
			var hM = hDate.getMonth() + 1;
			var hD = hDate.getDate();
			if (hY == solarY && hM == solarM && solarD <= hD + calendarData.Y_Holidays[j].a && solarD >= hD - calendarData.Y_Holidays[j].b) {
...
function toDay(date) {
	...
	var DLS = document.getElementsByTagName("ol")[0].childNodes;
	for (var i = 0; i < DLS.length; i++) {
		if (DLS[i].getAttribute("date") == date && !hasClass(DLS[i], "now"))
			addClass(DLS[i], "on");
		else
			removeClass(DLS[i], "on");
	}
